...
static SpeexHeader *header_of_value(value v, SpeexHeader *header)
{
  ...
  shv(header_size);
  shv(rate);
  header->mode = Int_val(caml_callback(*caml_named_value("caml_speex_int_of_mode"),
                           Field(v,i++)));
...
CAMLprim value value_of_header(SpeexHeader *header)
{
  ...
  svh(header_size);
  svh(rate);
  Store_field(ret,i++,caml_callback(*caml_named_value("caml_speex_mode_of_int"),
                          Val_int(header->mode)));
...
CAMLprim value ocaml_speex_encode_page(value e_state, value o_chans, value o_stream_state, value feed)
{
...
CAMLprim value ocaml_speex_encode_page_int(value e_state, value o_chans, value o_stream_state, value feed)
{
  CAMLparam3(e_state, feed, o_stream_state);
  CAMLlocal2(v,ret);
  ogg_stream_state *os = Stream_state_val(o_stream_state);
...
CAMLprim value ocaml_speex_decoder_decode(value e, value o_chans, value s, value add)
{
  CAMLparam3(e,s,add);
  CAMLlocal2(ret,v);
...
CAMLprim value ocaml_speex_decoder_decode_int(value e, value o_chans, value s, value add)
{
  CAMLparam3(e,s,add);
  CAMLlocal2(ret,v);
  ogg_stream_state *os = Stream_state_val(s);
